...
const FALLBACK_MORE = [
  { date: "October 15, 2025", category: "Community", title: "SACCO Membership Surpasses 3,500 Active Members", excerpt: "Our community savings and credit cooperative continues to grow, providing financial services to thousands of rural families." },
  { date: "September 28, 2025", category: "Agriculture", title: "New Disease-Resistant Coffee Varieties Introduced", excerpt: "BCC partners with research institutions to bring next-generation coffee varieties to Bugwere farmers, promising higher yields and better resilience." },
  { date: "August 10, 2025", category: "Partnership", title: "Partnership With District Government Expands Training Programs", excerpt: "A new memorandum of understanding with local government will double our training capacity across the Bugwere region." },
];
...
export default function NewsClient({ news: dbNews }: NewsClientProps) {
  // Split news into featured, articles, and more stories
  const featuredArticles = dbNews.filter(n => n.featured);
  const regularArticles = dbNews.filter(n => !n.featured);

  const featured = featuredArticles.length > 0
    ? { image: featuredArticles[0].image, date: featuredArticles[0].date, category: featuredArticles[0].category, title: featuredArticles[0].title, excerpt: featuredArticles[0].excerpt }
    : FALLBACK_FEATURED;

  const articles = regularArticles.length > 0
    ? regularArticles.slice(0, 2).map(a => ({ image: a.image, date: a.date, category: a.category, title: a.title, excerpt: a.excerpt }))
    : FALLBACK_ARTICLES;

  const moreStories = regularArticles.length > 2
    ? regularArticles.slice(2).map(a => ({ date: a.date, category: a.category, title: a.title, excerpt: a.excerpt }))
    : FALLBACK_MORE;
