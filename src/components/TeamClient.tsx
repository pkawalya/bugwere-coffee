...
export default function TeamClient({ team: dbTeam }: TeamClientProps) {
  const team = dbTeam.length > 0
    ? dbTeam.map(m => ({ name: m.name, role: m.role, bio: m.bio, color: m.color || (dbTeam.indexOf(m) % 2 === 0 ? SECONDARY : PRIMARY) }))
    : FALLBACK_TEAM;
